...
# Cargar el archivo Excel automáticamente desde la raíz del proyecto
excel_file_path = "DatosBD3.xlsx"  # Ruta del archivo Excel
df = pd.read_excel(excel_file_path)  # Cargar el Excel en un DataFrame
...
# Funcionalidad para realizar consultas sobre el archivo Excel (opcional, puede ser eliminada si no es necesaria)
elif seccion == "Consultas sobre Excel":
    
    
    # Estilos CSS para el título de la sección "Consultas sobre Excel"
    st.markdown(
        """
        <style>
        .excel-title {
            font-size: 30px;
            font-weight: bold;
            color: #0073e6; /* Color azul ajustado al tono del logo */
            text-align: center;
            margin-bottom: 20px;
        }
        .excel-subtitle {
            font-size: 24px;
            font-weight: bold;
            color: #43526d; /* Color gris-azulado para subtítulos */
            text-align: center;
            margin-bottom: 15px;
        }
        </style>
    ...
    # Aplicar el título estilizado en la sección "Consultas sobre Excel"
